//! Proxy transparente del Authorization Server OAuth 2.1 (Supabase
//! Auth) delante de `/mcp`: metadata de descubrimiento (RFC 9728),
//! `/authorize` + `/token` reenviados sin tocar el payload, y la
//! pantalla de consentimiento que Supabase Auth no aloja. Nada de
//! esto es parte del bridge MCP de `api/mcp.rs` — vive aquí porque,
//! en sí mismo, es un servidor de autorización completo, no solo la
//! validación de un token de recurso (eso sigue en [`crate::auth`]).
...
pub async fn protected_resource_metadata_handler(headers: HeaderMap) -> Response {
...
pub async fn authorize_proxy_handler(uri: Uri) -> Response {
    let issuer = match std::env::var("OAUTH_ISSUER") {
        Ok(v) => v,
...
pub async fn token_proxy_handler(headers: HeaderMap, body: Bytes) -> Response {
    let issuer = match std::env::var("OAUTH_ISSUER") {
        Ok(v) => v,
        Err(_) => return (StatusCode::NOT_FOUND, "OAUTH_ISSUER no configurado").into_response(),
    };
...
pub async fn consent_page_handler() -> Response {
    let issuer = match std::env::var("OAUTH_ISSUER") {
        Ok(v) => v,
        Err(_) => return (StatusCode::NOT_FOUND, "OAUTH_ISSUER no configurado").into_response(),
    };
